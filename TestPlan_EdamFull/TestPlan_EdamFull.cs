...
{
    public class EdamFull : TestPlanBase, IATFTest
    {
        ...
        public override string DoATFInit(string args)
        {
            ...
            else
            {
                ...
                if (m_modelTpState.Pa_Site)
                {
                    m_modelTpState.SetTesterSite(new Tester.PgEdamRf1Tester());
                }
            }
            #endregion Check Test Site
            byte SiteCount = Convert.ToByte(m_tcfReader.DicLocalfile["TEST_CONFIG"]["TEST_SITE_COUNT"]);
            StopWatchManager.Instance.NumSites = SiteCount;            
            Eq.SetNumSites(SiteCount);
            string[] EnabledSiteStrArray = m_tcfReader.DicLocalfile["TEST_CONFIG"]["ENABLED_SITES"].Split(',');
        ...
        public ATFReturnResult DoATFTest(string args)
        {
            ...
            #region StoponcontinueFail Part 2 of 2
            foreach (byte site in ResultBuilder.ValidSites)
            {
        ...
        private bool InitializePaTest(PaEquipmentInitializer eqInitModel)
        {
            ...
            bool isSuccess = ccm.CableCalibration(m_doClotho1.ClothoRootDir, "EDAM", false);
            ...
            vdo = m_modelTestRunner.InitializeAllPaTests();
            m_modelTpState.SetLoadFail(vdo);
            if (!vdo.IsValidated)
            {
            ...
            if (!m_modelTpState.Spara_Site)
            {
                ...
                string load_board_id = "LB-8240-081-NAN";
                string contactor_id = "LN-8240-NAN-NAN";
                ...
                {                  
                    ...
                    load_board_id = load_board_id.Contains("8240") ? load_board_id : "LB-8240-801-NAN";
                    all_load_board_id += load_board_id + "_";

                    contactor_id = Eq.Site[site].HSDIO.UNIO_EEPROMReadID(EqHSDIO.UNIO_EEPROMType.Socket, 2);
                    contactor_id = contactor_id.Contains("8240") ? contactor_id : "LN-8240-G46-NAN";
                    all_contactor_id += contactor_id + "_";
                }
                ...
                for (byte site = 0; site < Eq.NumSites; site++)
                {
                    ...
                    if (isGUCALSuccess[site] == false)
                    {
                        ...
                        string ERR = string.Format("Site{0} failed GUCAL. To continue testing, please ensure TestSite is disabled on Handler", site + 1);
        ...
        private void RunPaTest()
        {
            ...
            //if (!m_modelTpState.Spara_Site)
            //{
            //    StopWatchManager.Instance.Start("TP-DoATFTest-CheckData_BurnOTP_PassFlag");
            //    m_modelTestRunner.CheckData_BurnOTP_PassFlag();
            //    StopWatchManager.Instance.Stop("TP-DoATFTest-CheckData_BurnOTP_PassFlag");
