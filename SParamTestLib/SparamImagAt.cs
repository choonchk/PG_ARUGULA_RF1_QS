    ...
    public class SparamImagAt : SparamTestCase.TestCaseAbstract 
    {
        ...
        public override int RunTest()
        {
            ...
            if (usePrevious > 0)
            {
                ...
                rtnResult = tmpSparamRaw[TriggerID].SParamData[GetSparamIndex()].SParam[i_TargetFreqCnt].Imag;
            }
            else
            {
                ...
                if (blnInterStart)
                {
                    double p2 = tmpSparamRaw[TriggerID].SParamData[GetSparamIndex()].SParam[iTargetCnt + 1].Imag;
                    double p1 = tmpSparamRaw[TriggerID].SParamData[GetSparamIndex()].SParam[iTargetCnt].Imag;
                    rtnResult = p1 + (partialGradient * (p2 - p1));
                }
                else
                {
                    rtnResult = tmpSparamRaw[TriggerID].SParamData[GetSparamIndex()].SParam[iTargetCnt].Imag;
                }
            }
