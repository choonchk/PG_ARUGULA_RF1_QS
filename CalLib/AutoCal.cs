...
namespace CalLib
{
    ...
    public class AutoCal
    {
        ...
        private int SiteNo { get; set; }
        private int TotalSite = 1;

        public AutoCal(IATFTest myAtfTest, bool productionMode, int Site)
        {
        ...
        public void ShowForm()
        {
            Thread otherWindowHostingThread = new Thread(new ThreadStart(
                () => {
                    FrmFullAutoCal frm = new FrmFullAutoCal(this, SiteNo);

                    bool status = System.IO.Path.GetFileName(GU.correlationFilePath).ToUpper().Replace(".CSV", "") == GU.thisProductsGuStatus.IsCFFileVerify(SiteNo).ToUpper() ? true : false;

                    if ((!GU.thisProductsGuStatus.VerifyIsOptional(SiteNo) || !status) && ProductionMode)
                    {
                ...
                switch (calType)
                {
                    ...
                    case CalType.GU:
                        ...
                        if (Eq.Handler.Handler_Type == ATFRTE.Instance.HandlerType.ToString())
                            ExecuteAutoGUCal(ct, progressDetailMessage, progressPercentValue, SiteNo);
                        else
                            ExecuteGUCal(ct, progressDetailMessage, progressPercentValue, SiteNo);
        ...
        {
            Eq.Site[0].HSDIO.dutSlaveAddress = "F";
            // [Burhan]
            ...
            Eq.Handler.TrayMapTermination();
            */
            Eq.Site[0].HSDIO.dutSlaveAddress = "E";
            string a = "";

        }

        public Dictionary<string, Spec> Verified_Data;
        private void ExecuteAutoGUCal(CancellationToken ct, IProgress<string> progressObserver, IProgress<double> progressPercent, int SiteNo)
        {
            string err = "";
            List<GuSnSequence> GuTrayMapSeq = GetHandlerTrayMapSeq(GU.dutIdLooseUserReducedList.Count, TotalSite);
            ...
            if (GU.GuMode[site] == GU.GuModes.CorrVrfy ||
                GU.GuMode[site] == GU.GuModes.DCcheckCorrVrfy)
                numTrayInsertions = 2;
            ...
            try
            {
                using (CalHandlerProxy proxy = new CalHandlerProxy())
                {
                    ...
                    Tuple<bool, string> dutRet;

                    #region DC Check
                    if ((GU.GuMode[site] == GU.GuModes.DCcheckCorrVrfy) ||
                        (GU.GuMode[site] == GU.GuModes.DCcheckVrfy))
                    {
                        string dut_list = "Load DC check substrate into input tray and then press OK";
                        MessageBox.Show(dut_list, "Auto DC Check", MessageBoxButtons.OK, MessageBoxIcon.Information);

                        if (!GU.runningGU.Contains(true)) return;

                        if (ct.IsCancellationRequested) return;

                        dutRet = proxy.TheInstance_CAL.PickupDUT("");

                        if (dutRet.Item1)
                        {
                            /////////////// test DC substrate here /////////////////
                            Thread.Sleep(5);

                            foreach (KeyValuePair<string, EqLib.EqDC.iEqDC> value in Eq.Site[site].DC)
                            {
                                if (Eq.Site[site].HSDIO.IsMipiChannel(value.Key.ToUpper()))
                                {
                                    Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(3, 0.001);
                                }
                                else
                                {
                                    Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(3, 0.1);
                                }
                            }
                            ...
                            foreach (KeyValuePair<string, EqLib.EqDC.iEqDC> value in Eq.Site[site].DC)
                            {
                                if (Eq.Site[site].HSDIO.IsMipiChannel(value.Key.ToUpper()))
                                {
                                    Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(3, 0.000002);
                                    Eq.Site[site].DC[value.Key.ToString()].PreLeakageTest(new ClothoLibAlgo.DcSetting());
                                }
                                else
                                {
                                    Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(3, 0.00001);
                                    Eq.Site[site].DC[value.Key.ToString()].PreLeakageTest(new ClothoLibAlgo.DcSetting());
                                }
                            }

                            Dictionary<string, double[]> Leakage_Data = new Dictionary<string, double[]>();

                            Thread.Sleep(5000);

                            foreach (string pinName in Eq.Site[site].DC.Keys)
                            {
                                Eq.Site[site].DC[pinName].SetupCurrentTraceMeasurement(1, 0.0005, TriggerLine.None);
                                double[] dcLeakageTrace = Eq.Site[site].DC[pinName].MeasureCurrentTrace();
                                Leakage_Data.Add(pinName, dcLeakageTrace);

                                if (!Eq.Site[site].HSDIO.IsMipiChannel(pinName.ToUpper()))
                                    Eq.Site[site].DC[pinName].PostLeakageTest();
                            }

                            Chart frmChart = new Chart(Leakage_Data, site);

                            Dictionary<string, Spec> Verified_Data = frmChart.VerifyData();

                            foreach (Spec _Data in Verified_Data.Values)
                            {
                                if (_Data.Flag == true)
                                {
                                    Flag = true;
                            ...
                            MessageBox.Show(Flag == true ? "Failed" : "Passed", "DC Check", MessageBoxButtons.OK, MessageBoxIcon.Information);

                            if (Flag)
                            {
                                GU.runningDCVerify = true;
                                frmChart.DrawChart();
                                frmChart.ShowDialog();
                            }
                            ...
                            if (!dutRet.Item1)
                            {
                                err = string.Format("CAL Test Putback DUT #{0} Failure: {1}", 1, dutRet.Item2);
                                ATFLogControl.Instance.Log(LogLevel.Error, LogSource.eTestPlan, err);
                            }
                        }
                        else
                        {
                            err = string.Format("CAL Test Pickup DUT #{0} Failure: {1}", 1, dutRet.Item2);
                            ATFLogControl.Instance.Log(LogLevel.Error, LogSource.eTestPlan, err);
                        }
                    }
                    #endregion DC Check
                    ...
                    if (numTrayInsertions > 1 && hwInitRet.Item1)
                    {
                        ...
                        if (GU.GuMode[site] == GU.GuModes.DCcheckCorrVrfy)
                            GU.GuMode[site] = GU.GuModes.CorrVrfy;
                        ...
                        for (int trayInsertion = 0; trayInsertion < numTrayInsertions - 1; trayInsertion++)
                        {
                            ...
                            for (int deviceInsertion = 0; deviceInsertion < GuTrayMapSeq.Count; deviceInsertion++)
                            {
                                ...
                                if (dutRet.Item1)
                                {
                                    ...
                                    GU.StoreMeasuredData(ResultBuilder.results, site, true);
                    ...
                    if (hwInitRet.Item1)
                    {
                        GU.GuMode[site] = GU.GuModes.Vrfy;
                        ...
                        for (int deviceInsertion = 0; deviceInsertion < GuTrayMapSeq.Count; deviceInsertion++)
                        {
                            ...
                            if (dutRet.Item1)
                            {
                                ...
                                if (GU.GuVerifyFailureCount[site] == 0 ||   // first attempt or retry after passed
                                    (GU.GuVerifyFailureCount[site] > 0 && GU.dutIDfailedVrfy.Contains(GU.dutIdLooseUserReducedList[deviceInsertion])) ||    // retry after failed
                                    (GU.lastSelectedBatch != GU.selectedBatch)
                                   )
                                {
                                    myAtfTest.DoATFTest("");

                                    GU.StoreMeasuredData(ResultBuilder.results, site);
                                }
                                else
                                {
                                    Thread.Sleep(3000); // minimum time for PnP handler indexing
                                }
        ...
        private void ExecuteGUCal(CancellationToken ct, IProgress<string> progressObserver, IProgress<double> progressPercent, int SiteNo)
        {
            List<GuSnSequence> GuTrayMapSeq = GetHandlerTrayMapSeq(GU.dutIdLooseUserReducedList.Count, TotalSite);
                ...
                (GU.GuMode[site] == GU.GuModes.DCcheckVrfy))
            {
                ...
                {
                    string dut_list = "Insert DC check substrate into test socket and then press OK";
                    MessageBox.Show(dut_list, "DC Check", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                ...
                foreach (KeyValuePair<string, EqLib.EqDC.iEqDC> value in Eq.Site[site].DC)
                {
                    if (Eq.Site[site].HSDIO.IsMipiChannel(value.Key.ToUpper()))
                    {
                        Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(0.5, 0.001);
                    }
                    else
                    {
                        Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(0.5, 0.1);
                    }
                }
                ...
                foreach (KeyValuePair<string, EqLib.EqDC.iEqDC> value in Eq.Site[site].DC)
                {
                    if (Eq.Site[site].HSDIO.IsMipiChannel(value.Key.ToUpper()))
                    {
                        Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(0.5, 0.000002);
                        Eq.Site[site].DC[value.Key.ToString()].PreLeakageTest(new ClothoLibAlgo.DcSetting());
                    }
                    else
                    {
                        Eq.Site[site].DC[value.Key.ToString()].ForceVoltage(0.5, 0.00001);
                        Eq.Site[site].DC[value.Key.ToString()].PreLeakageTest(new ClothoLibAlgo.DcSetting());
                    }
                }
                ...
                {
                    Eq.Handler.TrayMapCoord(GuTrayMapSeq[0].HandlerSotCmd);
                    Eq.Handler.TrayMapEOT(GuTrayMapSeq[0].HandlerEotCmd);
                    Eq.Handler.CheckSRQStatusByte(72);
                }
            }
            #endregion DC Check
                ...
                Eq.Handler.Handler_Type != "MANUAL")
            {
                ...
                dut_list += " into input tray and then press OK";
                MessageBox.Show(dut_list, "GU Verification", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
